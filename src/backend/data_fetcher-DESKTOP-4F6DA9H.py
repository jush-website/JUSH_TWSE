...
class DataFetcher:
    ...
    def sync_if_needed(self):
        ...
        # 即使官方沒更新，若在盤中，嘗試用 YF 批次更新即時價格
        if 9 <= now.hour <= 14:
            self.update_cache_with_realtime()

    def update_cache_with_realtime(self):
        """利用 yfinance 批次取得所有快取個股的現價，實現盤中大盤與資金板塊即時更新"""
        try:
            import yfinance as yf
            import pandas as pd
            import pytz
            ...
            # 取得即時 1分K
            df = yf.download(symbols, period="1d", interval="1m", group_by="ticker", threads=True, progress=False)
            if df.empty: return
            ...
            with self._lock:
                for sid, sym in zip(sids, symbols):
                    try:
                        ticker_df = df if len(symbols) == 1 else (df[sym] if sym in df.columns.levels[0] else None)
                        if ticker_df is not None and not ticker_df.empty:
                            closes = ticker_df['Close'].dropna()
                            if not closes.empty:
                                rt_price = float(closes.iloc[-1])
                                
                                cache = self._official_cache[sid]
                                cache_date_str = cache.get("date")
                                
                                # 如果官方快取還停留在昨天，我們就先自己更新現價與漲跌幅
                                if cache_date_str != now_date_str:
                                    if 'yday_price' not in cache:
                                        cache['yday_price'] = cache['price']
                                        
                                    cache['price'] = rt_price
                                    # 維持舊的 date 標記，代表官方 OpenAPI 還沒更新，但價格已經是即時的
                                    if cache['yday_price'] and cache['yday_price'] > 0:
                                        cache['change_pct'] = round((rt_price - cache['yday_price']) / cache['yday_price'] * 100, 2)
                    except: pass
        except Exception as e:
            if self.logger: self.logger.error(f"Realtime cache update error: {e}")
    ...
    def get_global_markets(self):
        ...
        try:
            ...
            if "TAIEX" in self._official_cache:
                ...
                now_date_str = datetime.now(pytz.timezone("Asia/Taipei")).strftime("%Y-%m-%d")
                if not pd.isna(off.get("price")) and not pd.isna(off.get("change_pct")) and off.get("date") == now_date_str:
                    results["台股大盤"] = {
                        "price": off["price"],
