...
INTEGRATED_SYSTEM_PROMPT = (
    ...
    "你的工作是把這些「已經算好/整理好」的資訊，提煉成一份精簡的分段式繁體中文報告，"
    "格式嚴格如下（每段以【標題】起頭獨立一行，之後接該段內文，每段最多 2 句、"
    "40 字以內，只講該面向最重要的重點）：\n"
    "【技術面】...\n【籌碼面】...\n【分點動向】...\n【基本面】...\n【消息面】...\n【市場情緒】...\n【策略回測】...\n【整體結論】...\n"
    "嚴禁逐條複誦原始資料：不要列出每一則新聞標題、每一家分點名稱或每一天的"
    "數字，要歸納成結論（例如「外資賣壓明顯」而不是把五家券商名字念一遍、"
    "「近期利空新聞集中在處分虧損與空單增加」而不是逐則列出）。"
    "全文總長控制在 300 字以內。"
    ...
    "【市場情緒】根據 PTT 股板近期文章標題與推文數，客觀描述散戶討論的"
    "熱度與話題方向（推文數越高代表討論越熱），不要把鄉民看法當成事實。"
    "【策略回測】引用系統模擬的統計數字（勝率/平均報酬等）說明 CDP 當沖"
    "規則近期在這檔的表現，並強調是未計成本的歷史模擬、僅供參考、"
    "不代表未來績效。"
...
def generate_integrated_analysis(payload):
    ...
    lines.append("")
    lines.append("== 市場情緒（PTT 股板近期文章，格式：日期 標題(推文數)）==")
    lines.append(payload.get('community_summary') or "無資料")

    lines.append("")
    lines.append("== 策略回測（系統以固定規則對歷史日 K 的模擬結果）==")
    lines.append(payload.get('backtest_summary') or "無資料")
