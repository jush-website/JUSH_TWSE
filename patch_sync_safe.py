...
# Extract everything up to the function
start_idx = code.find('async def background_strategies_sync():')
# Find the end by looking for "app = FastAPI"
end_idx = code.find('app = FastAPI', start_idx)

# If we found it, replace just that part
if start_idx != -1 and end_idx != -1:
    new_code = code[:start_idx] + new_sync_code + "\n\n" + code[end_idx:]
